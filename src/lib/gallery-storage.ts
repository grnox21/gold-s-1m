 ...
 * (folder "barbers", see berberler/actions.ts and lib/barber-photos.ts) —
 * same bucket, same public-read policy (0012_gallery_storage.sql), just a
 * path prefix to keep the different kinds of image apart in the Storage
 * browser.
 */
export async function uploadImageToBucket(
  ...
  if (file.size > MAX_GALLERY_IMAGE_BYTES) {
    return { ok: false, error: `Görsel çok büyük — en fazla ${Math.floor(MAX_GALLERY_IMAGE_BYTES / (1024 * 1024))}MB olabilir.` };
  }
